...
bool CPegDB::LoadPegData(CTxDB& txdb, LoadMsg load_msg) {
	...
	// now process pegdb & votes if not ready
	{
		...
		if (nPegStartHeightStored != nPegStartHeight || fPegPruneStored != fPegPruneEnabled ||
		    !fPegCheck1 || !fPegCheck2 || !fPegCheck3) {
			...
			while (pblockindex && pblockindex->nHeight >= nPegStartHeight &&
				...
				for (CTransaction& tx : block.vtx) {
					...
					bool                   fInvalid = false;
					if (!tx.FetchInputs(txdb, pegdb, mapUnused, mapQueuedFractionsChanges, false,
					                    false, mapInputs, mapInputsFractions, fInvalid))
					...
					bool peg_ok = CalculateStandardFractions(
					    ...
					    mapInputsFractions, mapQueuedFractionsChanges, feesFractions,
					    sPegFailCause);
					if (!peg_ok) {
				...
				if (block.vtx.size() > 1 && block.vtx[1].IsCoinStake()) {
					...
					int64_t nCalculatedStakeReward = GetProofOfStakeReward(
					    pblockindex->pprev, nCoinAge, nFees, mapInputsFractions[fkey]);
					int64_t nStakeRewardWithoutFees = GetProofOfStakeReward(
					    pblockindex->pprev, nCoinAge, 0 /*fees*/, mapInputsFractions[fkey]);
					...
					bool peg_ok = CalculateStakingFractions(
					    tx, pblockindex, mapInputs, mapInputsFractions, mapUnused,
