...
bool CPegLevel::IsValid() const {
	return nBuffer >= 0 && nSupply >= 0 && nSupply < PEG_SIZE && nSupplyNext >= 0 &&
	       nSupplyNext < PEG_SIZE && nSupplyNextNext >= 0 && nSupplyNextNext < PEG_SIZE &&
	       nShift >= 0 && (nSupply + nShift) < PEG_SIZE && nShiftLastPart >= 0 &&
	       nShiftLastTotal >= 0;
}
