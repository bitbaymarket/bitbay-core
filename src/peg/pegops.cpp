...
bool getpeglevel(int                inp_cycle_now,
                 int                inp_cycle_prev,
                 int                inp_buffer,
                 int                inp_peg_now,
                 int                inp_peg_next,
	...
	getpeglevel(inp_cycle_now, inp_cycle_prev, inp_buffer, inp_peg_now, inp_peg_next,
	            inp_peg_next_next, pdExchange, pdPegShift,

	            peglevel, pdPegPool, out_err);
...
bool getpeglevelinfo(const std::string& inp_peglevel_hex,
...
bool getpegdatainfo(const std::string& inp_pegdata64,
...
bool updatepegbalances(const std::string& inp_balance_pegdata64,
	...
	bool ok = updatepegbalances(pdBalance, pdPegPool, peglevelNew, out_err);
	if (!ok) {
		return false;
	}
...
bool movecoins(int64_t            inp_move_amount,
	...
	bool ok = movecoins(inp_move_amount, pdSrc, pdDst, peglevel, inp_cross_cycles, out_err);
	if (!ok) {
		return false;
	}
...
bool moveliquid(int64_t            inp_move_liquid,
                const std::string& inp_src_pegdata64,
                const std::string& inp_dst_pegdata64,
                const std::string& inp_peglevel_hex,
	...
	bool ok = moveliquid(inp_move_liquid, pdSrc, pdDst, peglevel, out_err);
	if (!ok) {
		return false;
	}
...
bool movereserve(int64_t            inp_move_reserve,
                 const std::string& inp_src_pegdata64,
                 const std::string& inp_dst_pegdata64,
                 const std::string& inp_peglevel_hex,
	...
	bool ok = movereserve(inp_move_reserve, pdSrc, pdDst, peglevel, out_err);
	if (!ok) {
		return false;
	}
...
bool removecoins(const std::string& inp_from_pegdata64,
...
bool updatetxout(const std::string  inp_txout_pegdata64,
                 const std::string& inp_peglevel_hex,
	...
	out_txout_next_cycle_available_liquid_hli =
	    pdTxout.fractions.HighPart(pegn_liquid, nullptr).HLI();
	out_txout_next_cycle_available_reserve_hli =
	    pdTxout.fractions.LowPart(pegn_reserve, nullptr).HLI();

	return true;
}

bool prepareliquidwithdraw(const txinps&      inp_txinps,
	...
	std::vector<std::tuple<std::string, CPegData, std::string>> txIns;
	...
	bool ok =
	    prepareliquidwithdraw(txIns, pdBalance, pdExchange, pdPegShift, inp_amount_with_fee,
	                          inp_address, peglevel, pdRequested, pdProcessed, out_withdraw_idxch,
	                          out_withdraw_txout, out_rawtx, txOuts, out_err);
	...
	for (const std::tuple<string, CPegData, string>& txOut : txOuts) {
		...
		auto txout = make_tuple(std::get<0>(txOut), pdTxout.ToString(), std::get<2>(txOut));
...
bool preparereservewithdraw(const txinps&      inp_txinps,
                            const std::string  inp_balance_pegdata64,
                            const std::string  inp_exchange_pegdata64,
                            const std::string  inp_pegshift_pegdata64,
                            int64_t            inp_amount_with_fee,
	...
	bool ok =
	    preparereservewithdraw(txIns, pdBalance, pdExchange, pdPegShift, inp_amount_with_fee,
	                           inp_address, peglevel, pdRequested, pdProcessed, out_withdraw_idxch,
	                           out_withdraw_txout, out_rawtx, txOuts, out_err);
