...
public:
	...
	void refreshAddressTable() {
		...
		// qLowerBound() and qUpperBound() require our cachedAddressTable list to be sorted in asc
		// order
	...
	void updateEntry(const QString& address, const QString& label, bool isMine, int status) {
		...
		QList<AddressTableEntry>::iterator lower =
		    qLowerBound(cachedAddressTable.begin(), cachedAddressTable.end(), address,
		                AddressTableEntryLessThan());
		QList<AddressTableEntry>::iterator upper =
		    qUpperBound(cachedAddressTable.begin(), cachedAddressTable.end(), address,
		                AddressTableEntryLessThan());
		...
		switch (status) {
			case CT_NEW:
				if (inModel) {
					qDebug() << "AddressTablePriv::updateEntry : Warning: Got CT_NEW, but entry is "
					            "already in model";
					break;
				}
			...
			case CT_UPDATED:
				if (!inModel) {
					qDebug() << "AddressTablePriv::updateEntry : Warning: Got CT_UPDATED, but "
					            "entry is not in model";
					break;
				}
			...
			case CT_DELETED:
				if (!inModel) {
					qDebug() << "AddressTablePriv::updateEntry : Warning: Got CT_DELETED, but "
					            "entry is not in model";
					break;
				}
	...
	if (role == Qt::EditRole) {
		switch (index.column()) {
			...
			case Address:
				...
				// Check for duplicate addresses to prevent accidental deletion of addresses, if you
				// try to paste an existing address over another address (with a different label)
				...
				// Double-check that we're not overwriting a receiving address
				else if (rec->type == AddressTableEntry::Sending) {
					{
						LOCK(wallet->cs_wallet);
						// Remove old entry
						wallet->DelAddressBookName(
						    CBitcoinAddress(rec->address.toStdString()).Get());
						// Add new entry with new address
						wallet->SetAddressBookName(
						    CBitcoinAddress(value.toString().toStdString()).Get(),
