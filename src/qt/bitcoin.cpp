...
static void ThreadSafeMessageBox(const std::string& message,
...
#endif
		                           ,
		                       Qt::AlignBottom | Qt::AlignHCenter, QColor(0, 220, 240));
...
static void handleRunawayException(std::exception* e) {
	...
	QMessageBox::critical(
	    0, "Runaway exception",
	    BitcoinGUI::tr(
	        "A fatal error occurred. BitBay can no longer continue safely and will quit.") +
...
int main(int argc, char* argv[]) {
	...
	// Show help message immediately after parsing command-line options (for "-lang") and setting
	// locale, but before showing splash screen.
...
#ifdef Q_OS_MAC
	// on mac, also change the icon now because it would look strange to have a testnet splash
	// (green) and a std app icon (orange)
		...
		if (AppInit2(threadGroup)) {
			{
				...
				// bitcoin: URIs
				QObject::connect(paymentServer, SIGNAL(receivedURI(QString)), &window,
				                 SLOT(handleURI(QString)));
