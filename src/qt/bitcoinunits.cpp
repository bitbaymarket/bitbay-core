...
QString BitcoinUnits::description(int unit) {
	switch (unit) {
		case BTC:
			return QString("BitBay");
		case mBTC:
			return QString("Milli-BitBay (1 / 1,000)");
		case uBTC:
			return QString("Micro-BitBay (1 / 1,000,000)");
		default:
			return QString("???");
	}
}
...
QString BitcoinUnits::formatWithUnitForLabel(int unit, qint64 amount, bool plussign, int hli) {
	...
	if (hli < 0) {
		return QString("<b>") + format(unit, amount, plussign) +
		       QString("</b> <font color='#666666'>") + name(unit) + QString("</font>");
	}
	rate        = rate.leftJustified(4, 'x');
	rate        = rate.replace('x', "&nbsp;");
	QString txt = QString("<b>") + format(unit, amount, plussign) +
	              QString("</b> <font color='#666666' face='Roboto Mono'>") + name(unit) +
	              QString("<sup>%1</sup>").arg(rate) + QString("</font>");
	return txt;
}
