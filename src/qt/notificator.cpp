...
void Notificator::notifyDBus(Class          cls,
                             const QString& title,
                             const QString& text,
                             const QIcon&   icon,
                             int            millisTimeout) {
	...
	hints["icon_data"] =
	    FreedesktopImage::toVariant(tmpicon.pixmap(FREEDESKTOP_NOTIFICATION_ICON_SIZE).toImage());
	args.append(hints);
...
#ifdef Q_OS_MAC
void Notificator::notifyGrowl(Class          cls,
                              const QString& title,
                              const QString& text,
                              const QIcon&   icon) {
	const QString script(
	    ...
	    "  register as application \"%1\" all notifications allNotificationsList default "
	    "notifications enabledNotificationsList\n"  // -- Register our script with Growl
	    "  notify with name \"Notification\" title \"%2\" description \"%3\" application name "
	    "\"%1\"%4\n"  // -- Send a Notification
	    "end tell");
	...
	if (!notificationIconPixmap.isNull() && notificationIconFile.open()) {
		...
		if (writer.write(notificationIconPixmap.toImage()))
			notificationIcon =
			    QString(" image from location \"file://%1\"").arg(notificationIconFile.fileName());
	}
...
void Notificator::notify(Class          cls,
	...
	switch (mode) {
		...
		default:
			if (cls == Critical) {
				// Fall back to old fashioned pop-up dialog if critical and no other notification
				// available
