...
void RPCExecutor::request(const QString& command) {
	...
	try {
		...
		} catch (std::runtime_error&)  // raised when converting to invalid type, i.e. missing code
		                               // or message
		{                              // Show raw JSON object
			emit reply(RPCConsole::CMD_ERROR,
			           QString::fromStdString(write_string(json_spirit::Value(objError), false)));
		}
...
RPCConsole::RPCConsole(QWidget* parent) : QDialog(parent), ui(new Ui::RPCConsole), historyPtr(0) {
	ui->setupUi(this);
...
void RPCConsole::clear() {
	...
	for (int i = 0; ICON_MAPPING[i].url; ++i) {
		ui->messagesWidget->document()->addResource(
		    ...
		    QImage(ICON_MAPPING[i].source)
		        .scaled(ICON_SIZE, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
	}
	...
	message(CMD_REPLY,
	        (tr("Welcome to the BitBay RPC console.") + "<br>" +
