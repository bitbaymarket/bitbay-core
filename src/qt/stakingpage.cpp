...
StakingPage::StakingPage(QWidget* parent) : QDialog(parent), ui(new Ui::StakingPage) {
	ui->setupUi(this);
	...
	connect(ui->lineRewardTo, SIGNAL(textChanged(const QString&)), this,
	        SLOT(updateRewardActions()));
	...
	connect(ui->rewardAddressBookButton, SIGNAL(clicked(bool)), this,
	        SLOT(addressBookRewardClicked()));
	connect(ui->supportAddressBookButton, SIGNAL(clicked(bool)), this,
	        SLOT(addressBookSupportClicked()));
}
...
void StakingPage::updateTimer() {
	...
	if (nLastCoinStakeSearchInterval && nWeight) {
		...
		ui->labelText->setText(tr("Staking.<br>Your weight is %1<br>Network weight is "
...
void StakingPage::setWalletModel(WalletModel* model) {
	...
	if (model && model->getOptionsModel()) {
		...
		setAmounts(vRewardsInfo[PEG_REWARD_5].amount, vRewardsInfo[PEG_REWARD_10].amount,
		...
		connect(model,
		        SIGNAL(rewardsInfoChanged(qint64, qint64, qint64, qint64, int, int, int, int, int,
		                                  int, int, int)),
		        this,
		        SLOT(setAmounts(qint64, qint64, qint64, qint64, int, int, int, int, int, int, int,
		                        int)));
...
void StakingPage::setAmounts(qint64 amount5,
                             qint64 amount10,
                             qint64 amount20,
                             qint64 amount40,
                             int    count5,
