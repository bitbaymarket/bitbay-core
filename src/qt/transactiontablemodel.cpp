...
public:
	...
	void updateWallet(const uint256& hash, int status) {
		qDebug() << "TransactionTablePriv::updateWallet : " +
		                QString::fromStdString(hash.ToString()) + " " + QString::number(status);
		{
			LOCK2(cs_main, wallet->cs_wallet);
			...
			switch (status) {
				case CT_NEW:
					if (inModel) {
						qDebug() << "TransactionTablePriv::updateWallet : Warning: Got CT_NEW, but "
						            "transaction is already in model";
						break;
					}
					if (!inWallet) {
						qDebug() << "TransactionTablePriv::updateWallet : Warning: Got CT_NEW, but "
				...
				case CT_DELETED:
					if (!inModel) {
						qDebug() << "TransactionTablePriv::updateWallet : Warning: Got CT_DELETED, "
						            "but transaction is not in model";
						break;
					}
					...
					break;
				case CT_UPDATED:
					// Miscellaneous updates -- nothing to do, status update will take care of this,
					// and is only computed for visible transactions.
					break;
			}
		}
	}
	...
	switch (wtx->status.status) {
		...
		case TransactionStatus::Confirmed:
			if (wtx->status.depth > Params().MaxReorganizationDepth()) {
				status = tr("Confirmed (more than %1 confirmations)")
				             .arg(Params().MaxReorganizationDepth());
			} else {
		...
		case TransactionStatus::MaturesWarning:
			status =
			    tr("This block was not received by any other nodes and will probably not be "
			       "accepted!");
			break;
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord* wtx,
	...
	QString str = BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(),
...
QVariant TransactionTableModel::data(const QModelIndex& index, int role) const {
	...
	switch (role) {
		...
		case Qt::ForegroundRole:
			...
			if (index.column() == Amount && rec->type != TransactionRecord::Generated &&
